...
import 'package:tanipedia_mobile/feature/edukasi/controller/c_edukasi_video_main/c_edukasi_video_main_state.dart';
import 'package:tanipedia_mobile/repositories/r_edukasi/r_edukasi.dart';

class CEdukasiVideoMain extends GetxController {
  static CEdukasiVideoMain instance = Get.find();
  late TabController tabController;
  final Rx<CEdukasiVideoMainState> _state = Rx<CEdukasiVideoMainState>(const CEdukasiVideoMainState());
  CEdukasiVideoMainState get state => _state.value;
  ...
  void onGetData() async {
    try {
      ...
      final response = await rEdukasi.getVideoCategories(limit: state.limit, offset: state.offset, search: searchText.text);
